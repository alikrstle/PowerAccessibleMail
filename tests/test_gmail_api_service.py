...
from accessible_mail.gmail_api_service import (
    MAX_GMAIL_API_RESPONSE_BYTES,
    GmailApiService,
)
...
class GmailApiServiceTests(unittest.TestCase):
    def setUp(self) -> None:
        # Service unit tests must never open or mutate the user's real mail cache.
        patcher = patch("accessible_mail.gmail_api_service.MessageCache", return_value=Mock())
        patcher.start()
        self.addCleanup(patcher.stop)

    @patch("accessible_mail.gmail_api_service.urllib.request.urlopen")
    def test_oversized_gmail_response_is_rejected(self, urlopen: Mock) -> None:
        response = Mock()
        response.headers = {"Content-Length": str(MAX_GMAIL_API_RESPONSE_BYTES + 1)}
        urlopen.return_value.__enter__.return_value = response
        urlopen.return_value.__exit__.return_value = False
        service = GmailApiService(cache=Mock())
        service._oauth_token = Mock(return_value="token")

        with self.assertRaisesRegex(MailError, "512 ميغابايت"):
            service._request_json(
                Account(id="account", email_address="user@example.com"),
                "GET",
                "https://gmail.googleapis.com/gmail/v1/users/me/messages/1",
            )
    ...
    def test_remote_gmail_attachment_is_kept_lazy_until_download(self) -> None:
        service = GmailApiService(cache=Mock())
        service._attachment_bytes = Mock(side_effect=AssertionError("must stay lazy"))
        message = {
            "id": "remote-message",
            "internalDate": "1760000000000",
            "labelIds": ["INBOX"],
            "payload": {
                ...
                "parts": [
                    {"mimeType": "text/plain", "body": {"data": "SGVsbG8"}},
                    {
                        ...
                        "body": {"attachmentId": "attachment-123", "size": 314572800},
                    },
                ],
            },
        }
        ...
        content = service._content_from_message(Account(), summary, message)

        attachment = next(item for item in content.links if item.is_attachment)
        self.assertEqual(attachment.remote_source, "gmail")
        self.assertEqual(attachment.remote_id, "attachment-123")
        self.assertEqual(attachment.size, 314572800)
        self.assertEqual(attachment.attachment_bytes(), b"")
        service._attachment_bytes.assert_not_called()

    def test_remote_gmail_attachment_download_reports_progress_and_writes_file(self) -> None:
        service = GmailApiService(cache=Mock())
        service._attachment_bytes = Mock(return_value=b"downloaded attachment")
        ...
        summary = MessageSummary(uid="message-1", mailbox="INBOX")
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(result.read_bytes(), b"downloaded attachment")
        self.assertEqual(progress[-1], (21, 21))
    ...
    def test_gmail_snippet_is_used_when_message_body_has_no_visible_text(self) -> None:
        service = GmailApiService()
        message = {
            "id": "image-only-message",
            "snippet": "An accessible preview of the image-only message.",
            "internalDate": "1760000000000",
            "labelIds": ["INBOX"],
            "payload": {
                "headers": [],
                ...
                "body": {
                    "data": base64.urlsafe_b64encode(
                        b'<html><body><img src="https://example.com/banner.png"></body></html>'
                    )
                    .decode("ascii")
                    .rstrip("=")
                },
        ...
        self.assertEqual(
            content.text,
            "An accessible preview of the image-only message.",
        )
    ...
    def test_quoted_printable_html_body_is_decoded_before_parsing(self) -> None:
        service = GmailApiService()
        quoted_html = (
            b'<html><body><h1 style=3D"color: red">ASUS invitation</h1>'
            b'<p>Priority access is now open.</p></body></html>'
        )
        message = {
            "id": "asus-quoted-printable",
            "internalDate": "1760000000000",
            "labelIds": ["INBOX", "UNREAD"],
            "payload": {
                ...
                "parts": [
                    ...
                    },
                    {
                        ...
                        "body": {
                            "data": base64.urlsafe_b64encode(quoted_html)
                            .decode("ascii")
                            .rstrip("="),
                            "size": len(quoted_html),
                        },
                    },
                ],
            },
